...
class ModelBasedTD3(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.policy_observation_space = spaces.Box(
            low = -1,
            high = 1,
            shape = (self.observation_space.shape[0] + dynamics_model_pred_len * dynamics_model.feature_size ,)
    ...
    def get_prices(self, obs):
            stock_dims = self.dynamics_model.feature_size
            prices = obs[:, 1:1+stock_dims]
            assert prices.shape[-1] == self.dynamics_model.feature_size, f'Prices didnt come out the right shape, stock dim: {self.dynamics_model.feature_size}, obs: {obs.shape}, prices: {prices.shape}'
            return prices
